...
v_curr = np.zeros(3)  # current speed (stationary, measured) # [TEMP]
v_wind = np.zeros(3)  # wind speed (stationary, measured) # [TEMP]
b_curr = np.zeros(6)  # current force # [TEMP] --> get from dist.current()
b_wind = np.zeros(6)  # wind force # [TEMP] --> get from dist.wind()
    ...
    if ctrl_t == 0.0 or ctrl_t >= ctrl_dt:
        ...
        if VERBOSE and i != 0:
            ...
            print(
                ...
                f"\n\tb_curr =\t[{b_curr[3]:.4f}, {b_curr[4]:.4f}, {b_curr[5]:.4f}]"
                f"\n\tb_wind =\t[{b_wind[3]:.4f}, {b_wind[4]:.4f}, {b_wind[5]:.4f}]"
                f"\n\tcost mpc =\t{cost:.4f}"
            )
