...
model.set_initial_conditions(np.zeros(6))  # set the initial conditions for the model

# Initialize disturbances
dist = disturbances.Disturbances()
dist.set_current_direction(-3 / 4 * np.pi)  # set the current direction [rad]
dist.set_current_speed(0.3)  # set the current speed [m/s] (max speed is 1.0 m/s)
dist.set_wind_direction(-3 / 4 * np.pi)  # set the wind direction [rad]
dist.set_wind_speed(0.3)  # set the wind speed [m/s]
...
u[0] = 100  # left actuator
u[1] = 100  # right actuator
u[2] = 0  # angle left
u[3] = 0  # angle right
