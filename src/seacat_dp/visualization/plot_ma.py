...
def plot_variables(
    ...
    cols = ["SeaCAT", "SeaDragon"]
    for axis, col in zip(ax[0], cols):
        axis.set_title(col)

    # SEACAT ###########################
    # Plot input forces
    ax[0, 0].plot(t_vec, u_mat[0, :])
    ax[0, 0].plot(t_vec, u_mat[1, :])
    ax[0, 0].plot(t_vec, u_mat[2, :])
    ax[0, 0].plot(t_vec, u_mat[3, :])
    ax[0, 0].set(xlabel="", ylabel="$u$ [N]")
    ax[0, 0].grid()
    ax[0, 0].set_axisbelow(True)

    ax[0, 0].legend(["$u_1$", "$u_2$", "$u_3$", "$u_4$"])

    # Plot x, y
    ax[1, 0].plot(t_vec, q_mat[0, :])
    ax[1, 0].plot(t_vec, q_mat[1, :])
    if q_ref_mat is not None:
        ax[1, 0].plot(t_vec, q_ref_mat[0, :], "--", color="k")
        ax[1, 0].plot(t_vec, q_ref_mat[1, :], "--", color="k")
    ax[1, 0].set(xlabel="", ylabel="$x, y$ [m]")
    ax[1, 0].grid()
    ax[1, 0].set_axisbelow(True)
    ax[1, 0].legend(["$x$", "$y$"])

    # Plot theta
    ax[2, 0].plot(t_vec, q_mat[2, :])
    if q_ref_mat is not None:
        ax[2, 0].plot(t_vec, q_ref_mat[2, :], "--", color="k")
    ax[2, 0].set(xlabel="", ylabel=r"$\psi$ [rad]")
    ax[2, 0].grid()
    ax[2, 0].set_axisbelow(True)
    ax[2, 0].set_ylim(-np.pi, np.pi)

    # Plot u, v
    ax[3, 0].plot(t_vec, q_mat[3, :])
    ax[3, 0].plot(t_vec, q_mat[4, :])
    ax[3, 0].set(xlabel="", ylabel=r"$u, v$ [m/s]")
    ax[3, 0].grid()
    ax[3, 0].set_axisbelow(True)
    ax[3, 0].legend(["$u$", "$v$"])

    # Plot omega
    ax[4, 0].plot(t_vec, q_mat[5, :])
    ax[4, 0].set(xlabel="time [s]", ylabel=r"$\omega$ [rad/s]")
    ax[4, 0].grid()
    ax[4, 0].set_axisbelow(True)

    # Plot cost
    if cost_mat is not None:
        ax[5, 0].plot(t_vec, cost_mat)
        ax[5, 0].set(xlabel="time [s]", ylabel="cost")
        ax[5, 0].grid()
        ax[5, 0].set_axisbelow(True)

    # SEADRAGON ###########################
    # Plot input forces
    h1 = ax[0, 1].plot(t_vec, u_mat[4, :])
    h2 = ax[0, 1].plot(t_vec, u_mat[5, :])
    h3 = ax[0, 1].plot(t_vec, u_mat[6, :])
    h4 = ax[0, 1].plot(t_vec, u_mat[7, :])
    ax[0, 1].set(xlabel="", ylabel="$u$ [N]")
    # ax[0, 1].grid()
    # ax[0, 1].set_axisbelow(True)
    # ax2 = ax[0, 1].twiny()
    # ax2.set_xlim(ax[0, 1].get_xlim())
    # h3 = ax2.plot(t_vec, u_mat[6, :])
    # h4 = ax2.plot(t_vec, u_mat[7, :])
    # ax2.set(xlabel="", ylabel="$u$ [rad]")
    ax[0, 1].legend([h1[0], h2[0], h3[0], h4[0]], ["$u_1$", "$u_2$", "$u_3$", "$u_4$"])

    # Plot x, y
    ax[1, 1].plot(t_vec, q_mat[6, :])
    ax[1, 1].plot(t_vec, q_mat[7, :])
    if q_ref_mat is not None:
        ax[1, 1].plot(t_vec, q_ref_mat[6, :], "--", color="k")
        ax[1, 1].plot(t_vec, q_ref_mat[7, :], "--", color="k")
    ax[1, 1].set(xlabel="", ylabel="$x, y$ [m]")
    ax[1, 1].grid()
    ax[1, 1].set_axisbelow(True)
    ax[1, 1].legend(["$x$", "$y$"])

    # Plot theta
    ax[2, 1].plot(t_vec, q_mat[8, :])
    if q_ref_mat is not None:
        ax[2, 1].plot(t_vec, q_ref_mat[8, :], "--", color="k")
    ax[2, 1].set(xlabel="", ylabel=r"$\psi$ [rad]")
    ax[2, 1].grid()
    ax[2, 1].set_axisbelow(True)
    ax[2, 1].set_ylim(-np.pi, np.pi)

    # Plot u, v
    ax[3, 1].plot(t_vec, q_mat[9, :])
    ax[3, 1].plot(t_vec, q_mat[10, :])
    ax[3, 1].set(xlabel="", ylabel=r"$u, v$ [m/s]")
    ax[3, 1].grid()
    ax[3, 1].set_axisbelow(True)
    ax[3, 1].legend(["$u$", "$v$"])

    # Plot omega
    ax[4, 1].plot(t_vec, q_mat[11, :])
    ax[4, 1].set(xlabel="time [s]", ylabel=r"$\omega$ [rad/s]")
    ax[4, 1].grid()
    ax[4, 1].set_axisbelow(True)

    # Plot cost
    if cost_mat is not None:
        ax[5, 1].plot(t_vec, cost_mat)
        ax[5, 1].set(xlabel="time [s]", ylabel="cost")
        ax[5, 1].grid()
        ax[5, 1].set_axisbelow(True)
        ...
        for i in idx:
            if i >= q_mat.shape[1]:
                if i == q_mat.shape[1]:
                    ...
                    raise ValueError(
                        f"Index {i} is out of bounds for q_mat with shape "
                        f"{q_mat.shape}."
                    )
